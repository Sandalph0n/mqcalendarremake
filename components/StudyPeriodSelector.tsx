						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<label className="flex flex-col gap-2">
								...
								>
									...
									{range(Settings.period_year_min, Settings.period_year_max+1).map((y)=>(
										<option key = {String(y) + "year"} value={y}>{y}</option>
									))}
									{/* {weekOptions.map((opt) => (
							...
							<label className="flex flex-col gap-2">
								...
								>
									...
									{range(Settings.period_session_min, Settings.period_session_max+1).map((s)=>(
										<option key = {String(s) + "session"} value={s}>{s}</option>
									))}
									{/* {weekOptions.map((opt) => (
										<option key={opt.value} value={opt.value}>
											{opt.label}
