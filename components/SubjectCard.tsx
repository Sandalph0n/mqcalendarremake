	...
	return (
		<div className="relative w-full overflow-hidden rounded-xl border border-border/60 bg-muted/20 shadow-none transition-colors hover:border-primary/40">
			...
			<Dialog open={isEditing} onOpenChange={handleEditingOpenChange} >
				<VisuallyHidden.Root>
					<DialogTitle>SubjectCard Editor Box</DialogTitle>
				</VisuallyHidden.Root>
				...
				<DialogContent className={cn("fixed p-0 max-w-6xl w-full border-none bg-transparent shadow-none sm:max-w-5xl sm:max-h-5xl max-h-[90%] h-full")} showCloseButton={false}>
					<Card className="pt-0 w-full max-h-[90vh] ">
							...
							<div className="flex flex-1 flex-col gap-2 rounded-xl p-1 text-left transition-colors">
								...
								<div className="flex flex-wrap items-center gap-2 text-[11px] mt-1">
									{editingAssessments.length > 0 && (
										<span className="flex items-center gap-1 rounded-md bg-accent px-2 py-1 text-[11px] font-medium text-accent-foreground">
											<ClipboardList className="size-3.5" />
									...
									{editingNotFinishedAsm > 0 && (
										<span className="flex items-center gap-1 rounded-md bg-destructive px-2 py-1 text-[11px] font-medium text-accent-foreground">
											<ClipboardList className="size-3.5" />
						...
						<div className="w-full px-5">
							<div className="w-full rounded-xl border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
								<div className="flex items-start gap-2">
									<Info className="mt-0.5 size-4 text-primary" />
									<div className="space-y-0.5">
										<p className="font-semibold text-foreground">Instruction</p>
										<p>
											Please select either a due date or a due week in Unit Guide for each assessment.
										</p>
									</div>
								</div>
							</div>
